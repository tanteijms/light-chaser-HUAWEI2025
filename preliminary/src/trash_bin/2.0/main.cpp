...
int main()
{
    ...
    while (total_remaining_cnt > 0)
    {
        ...
        for (int i = 0; i < M; ++i)
        {
            ...
            // --- 修改：构建更丰富的候选服务器列表 ---
            std::vector<int> candidate_servers = best_servers_for_user[i];
            int last_server_idx = users[i].last_server_id - 1;
            ...
            for (int server_idx : candidate_servers)
            {
                ...
                CostInfo current = calculate_cost(users[i], server_idx, npu);
        ...
        if (best_user_idx != -1)
        {
            ...
            solution[best_user_idx].push_back({user.next_send_time, server.id, chosen_npu.id_in_server, best_B});
