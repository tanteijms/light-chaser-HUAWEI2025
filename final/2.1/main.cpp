...
struct Server
{
    ...
    int max_b[501]; // 预计算每个服务器的最大batch size
};
...
// 为给定服务器计算最佳批处理大小
int calculate_optimal_batch(const Server &server, int max_samples,int user_id)
{
    int max_b = std::min(max_samples, server.max_b[user_id]);
    if (max_b <= 1)
        return max_b;
...
void read_input() // 读取、解析输入
{
    ...
    for (int i = 0; i < N; ++i)
    {
        for (int j=1;j<=M;j++) {
            if (a[j]==0) {
                servers[i].max_b[j]=1000;
            }
            else {
                servers[i].max_b[j]=std::min(1000,(servers[i].m-b[j])/a[j]); // 计算后直接存入Server结构体
            }
        }
    }
